...
bool Replacement::access_page(int page_num, bool is_write)
{
    ...
    if(!page_table[page_num].valid) // if page num isnt valid then enter to load or replace page
    {
        page_faults++; // increment page fault
        if(num_frames < max_frames) // if its less than max_frames then load page 
        {
            load_page(page_num);
            num_frames++;
            return true;
        }
        else // else replace page
        {
            page_replacements++;
    ...
    touch_page(page_num); // only for LRU will have nothing in FIFO/LIFO
